...
def by_num_cnot(k: int) -> typing.List[qiskit.QuantumCircuit]: # return a list quantum circuits
    """
    ...
    def by_num_cnot_func(qc: qiskit.QuantumCircuit):  
        ...
        for i in range(len(qc)):   # len(qc) - count the operations of qc
            if qc[i][0].name == 'cx':
                cnot_count += 1
...
def by_depth(depth: int) -> typing.List[qiskit.QuantumCircuit]:
    ...
    def by_depth_func(qc: qiskit.QuantumCircuit):
        def look_forward(qc, x):   # Add X (H, CNOT, X,...) gate into qc -> return new qc
            qc.append(x[0], x[1])
            return qc
        standard_depth = qc.depth()
        qc1 = qiskit.QuantumCircuit(qc.num_qubits)
